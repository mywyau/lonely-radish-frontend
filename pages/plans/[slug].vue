...
const times = [futureTime(2, 18, 30), futureTime(4, 14), futureTime(5, 15, 30)]
...
function timeLabel(value: string) { return times.find(time => time.value === value)?.label || value }
async function sendProposal() {
  sending.value = true
  sendError.value = ''
  try {
    await $fetch('/api/proposals', { method: 'POST', body: { profileSlug: String(route.params.slug), activity: activity.value,
...
onMounted(async () => {
  try {
    const response = await $fetch<{ person: { name: string }; activities: string[] }>(`/api/planning/${String(route.params.slug)}`)
