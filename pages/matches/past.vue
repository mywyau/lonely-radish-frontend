      ...
      <div v-else-if="connections.length" class="mt-8 grid gap-3">
        <article v-for="connection in connections" :key="connection.id" class="rounded-lg bg-white p-5 shadow-[0_8px_20px_rgba(180,35,74,.07)]">
          ...
          <div class="mt-4 flex flex-wrap gap-2">
            ...
            <NuxtLink v-if="connection.apologySent || connection.contactSent" :to="{ path: `/profiles/${connection.slug}`, query: { connection: 'past' } }" class="rounded-lg bg-[#B4234A] px-4 py-2.5 text-sm font-semibold text-white">Re-offer interest</NuxtLink>
          ...
          <form v-if="contactFor === connection.id" class="mt-4 rounded-lg bg-[#FBF7F1] p-4" @submit.prevent="sendContact(connection)"><label class="text-sm font-semibold">Message to {{ connection.name }} <span class="font-normal text-[#6E4D58]">(private message, not an apology)</span><textarea v-model="contactMessage" maxlength="500" rows="3" class="mt-2 w-full rounded-lg border border-[#D8C8B6] bg-white p-3 font-normal" placeholder="Say what you would like them to know, without pressure." /></label><p class="mt-2 text-xs text-[#6E4D58]">{{ connection.name }} can read this privately and does not need to respond. Send this message before re-offering interest.</p><div class="mt-3 flex gap-2"><button type="submit" :disabled="contactSending || !contactMessage.trim()" class="rounded-lg bg-[#8F1839] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{{ contactSending ? 'Sending…' : 'Send message' }}</button><button type="button" class="px-3 py-2 text-sm font-semibold text-[#6E4D58]" @click="contactFor = null">Cancel</button></div></form>
        </article>
