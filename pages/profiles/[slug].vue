...
async function sendContact() {
  if (!profile.value?.matchId || !contactMessage.value.trim()) return
    ...
    <section v-else-if="profile" class="mx-auto max-w-5xl">
      <div class="flex min-w-0 flex-col gap-5 lg:grid lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:items-start">
        ...
        <div class="contents lg:flex lg:flex-col lg:gap-5">
                ...
                <div v-else-if="profile.relationshipStatus === 'unmatched' && !profile.interestSent"
                  ...
                  <p v-else-if="profile.apologySent" class="mt-2 font-semibold text-[#6E8B52]">Your private note has
                   ...
                   <p v-else-if="profile.contactSent" class="mt-2 font-semibold text-[#6E8B52]">Your message was sent.
                     They may not necessarily respond, and you can now show interest in them again.</p>
          ...
          <ProfileAvailabilityContactCard v-if="availabilityIsVisible" v-model:flipped="availabilityContactFlipped"
            class="order-4" :availability="profile.availability"
            :contact-details="hasSharedContactDetails ? profile.contactDetails : null" :profile-name="profile.name" />
