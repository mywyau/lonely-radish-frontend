...
<template>
  <main class="min-h-[70vh] bg-[#FBF7F1] px-5 py-16 text-[#2A1520]">
    <section class="mx-auto max-w-2xl rounded-lg bg-white p-6 text-center shadow-[0_12px_30px_rgba(180,35,74,0.08)] sm:p-8">
      ...
      <h1 class="mt-6 text-3xl font-semibold">{{ activation === 'active' ? 'Your supporter membership is active' : 'Payment received' }}</h1>
      <p v-if="activation === 'checking'" class="mt-3 text-[#6E4D58]">We’re securely confirming your supporter membership. This normally takes only a few seconds.</p>
      <p v-else-if="activation === 'active'" class="mt-3 text-[#6E4D58]">Thank you. Stripe confirmed your support and your Lonely Radish account has been updated. Your visibility, ranking and core features remain the same as everyone else’s.</p>
      ...
      <div class="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
        ...
        <NuxtLink :to="activation === 'active' ? '/account/v2' : '/matches'" class="rounded-lg bg-[#2A1520] px-5 py-3 text-sm font-semibold text-white">{{ activation === 'active' ? 'View my membership' : 'Continue to matches' }}</NuxtLink>
      </div>
    </section>
  </main>
</template>
