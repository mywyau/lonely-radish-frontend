...
async function declineInterest(person: ReceivedInterest) {
  const decision = person.reconnectRequest ? 'reconnect' : 'match'
  if (declining.value || !window.confirm(`Decide not to ${decision} with ${person.name}? You will have 30 seconds to undo.`)) return
  ...
  try {
    ...
    successMessage.value = person.reconnectRequest
      ? `You chose not to reconnect with ${person.name}.`
      : `You chose not to match with ${person.name}. That space is now available for someone new.`
...
<template>
  <main class="min-h-screen bg-[#FBF7F1] px-5 py-10 text-[#2A1520] sm:px-8"><section class="mx-auto max-w-3xl">
    ...
    <div v-if="undoableDeclines.length" class="mt-4 grid gap-2">
      <UndoActionNotice v-for="action in undoableDeclines" :key="action.person.id" :message="action.person.reconnectRequest ? `You chose not to reconnect with ${action.person.name}.` : `You chose not to match with ${action.person.name}.`" :expires-at="action.undoUntil" :busy="action.restoring" @undo="undoDecline(action)" />
    </div>
    <p v-if="yourMoveMatch" class="mt-5 rounded-lg bg-[#FFF1C7] p-4 text-sm leading-6 text-[#694C00]" role="status"><strong>Your new connection with {{ yourMoveMatch.name }} needs attention.</strong> Send a date plan or close the connection before accepting another interest. You can still view profiles, pass and show interest in other people. <NuxtLink to="/matches" class="font-semibold underline">Go to connection</NuxtLink></p>
    ...
    <section v-if="!loading && closedInterests.length" class="mt-10" aria-labelledby="closed-interests-title">
      <h2 id="closed-interests-title" class="text-xl font-semibold">Expired or withdrawn interests</h2>
      <p class="mt-1 text-sm leading-6 text-[#6E4D58]">These interests expired or were taken back. You can still view the profile or use the safety options.</p>
