...
onMounted(async () => {
  try {
    const result = await $fetch<{ interests: ReceivedInterest[]; activeMatchCount: number; activeMatchLimit: number }>('/api/interests/received')
...
<template>
  ...
  <div v-if="pending" class="fixed inset-0 z-50 flex items-center justify-center bg-[#2A1520]/55 p-5" @click.self="pending = null"><section role="alertdialog" aria-modal="true" aria-labelledby="accept-title" class="w-full max-w-md rounded-xl bg-white p-6"><h2 id="accept-title" class="text-2xl font-semibold">Match with {{ pending.name }}?</h2><p class="mt-3 text-sm leading-6 text-[#6E4D58]">You will both be notified and can start planning a date. This will use one of your {{ activeMatchLimit }} active match spaces.</p><p v-if="atMatchLimit" class="mt-3 rounded-lg bg-[#FFF1C7] p-3 text-sm font-semibold text-[#694C00]">You have reached your active match limit. Complete or remove a match first.</p><div class="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button class="rounded-lg bg-[#F3E8DA] px-4 py-2.5 text-sm font-semibold" :disabled="accepting" @click="pending = null">Not now</button><button class="rounded-lg bg-[#B4234A] px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50" :disabled="accepting || atMatchLimit" @click="acceptInterest">{{ accepting ? 'Matching…' : 'Yes, create match' }}</button></div></section></div>
  </main>
</template>
