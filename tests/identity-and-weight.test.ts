...
describe('sexual orientation and profile weight', () => {
  it('stores and filters sexual orientation separately from gender', () => {
    ...
    const preferencePage = read('pages/preferences/dating.vue')
    expect(preferencePage).toContain('!preferences.orientations.length')
    expect(preferencePage).toContain('All genders')
    expect(preferencePage).not.toContain('>Everyone</button>')
    expect(read('server/api/activities/people.get.ts')).toContain("preferences.openToEveryone ? 'All genders'")
