...
describe('private profile photo storage', () => {
  ...
  it('validates, verifies, signs, reorders, and deletes uploaded photos', () => {
    ...
    expect(uploadUrl).toContain("contentType === 'image/jpeg'")
    expect(uploadUrl).toContain('thumbnailContentType')
    expect(read('server/api/profile/photos/confirm.post.ts')).toContain('.info(storageKey)')
    expect(read('server/api/profile/photos/confirm.post.ts')).toContain("new Set(['image/webp', 'image/jpeg'])")
    expect(read('server/api/profile/photos/confirm.post.ts')).toContain('sanitizeProfilePhoto')
    expect(read('server/api/profile/photos/confirm.post.ts')).toContain('bucket.download(storageKey)')
    expect(read('server/api/profile/photos/confirm.post.ts')).toContain('bucket.update(storageKey, normalizedPhoto')
    ...
    expect(page).toContain('contentType: optimized.full.type')
    ...
    expect(page).toContain('altText: `Profile photo ${photos.value.length + 1}`')
    expect(page).not.toContain('altText: `${file.name} profile photo`')
    ...
    expect(profilePage).toContain('class="order-2 hidden lg:block"')
    expect(profilePage.indexOf('class="order-2 hidden lg:block"'))
      .toBeLessThan(profilePage.indexOf('profile-summary-flip order-2'))
    const preview = read('pages/profile/preview.vue')
    expect(preview.match(/class="contents lg:flex lg:flex-col lg:gap-5"/g)).toHaveLength(2)
    expect(preview).toContain('class="order-2 hidden lg:block"')
    expect(preview.indexOf('aria-label="Your profile photos"'))
      .toBeLessThan(preview.indexOf('class="order-2 hidden lg:block"'))
