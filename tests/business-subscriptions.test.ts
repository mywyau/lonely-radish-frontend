...
describe('business subscriptions', () => {
  it('keeps business accounts, venues, offers and subscriptions separate', () => {
    const migration = read('docs/migrations/20260814_add_business_subscriptions.sql')
    ...
    expect(migration).toContain('create table if not exists business_subscriptions')
  ...
  it('uses ownership checks throughout protected business APIs', () => {
    expect(read('server/utils/requireBusiness.ts')).toContain('business_members')
    expect(read('server/api/business/offers/index.post.ts')).toContain('requireBusiness(event)')
    expect(read('server/api/business/portal.post.ts')).toContain('requireBusiness(event)')
  })
  ...
  it('starts Stripe checkout with isolated business metadata', () => {
    ...
    expect(processor).toContain("subscription.metadata?.subscriptionKind === 'business'")
  ...
  it('provides onboarding, offers and pricing pages', () => {
    expect(read('pages/business/index.vue')).toContain('Create your business profile')
    expect(read('pages/business/offers.vue')).toContain('Create an offer')
    expect(read('pages/business/pricing.vue')).toContain('Business subscriptions')
  ...
  it('separates business authentication and dating navigation', () => {
    const migration = read('docs/migrations/20260815_add_business_account_type.sql')
    ...
    expect(businessNav).toContain("label:'Dashboard'")
    expect(businessNav).toContain("label:'Offers'")
    expect(businessNav).not.toContain('Matches & plans')
    expect(businessNav).not.toContain('Match preferences')
    expect(read('middleware/account-mode.global.ts')).toContain("mode.accountType === 'business'")
    ...
    expect(read('pages/business/index.vue')).toContain('Complete your business profile before using the rest of the business portal.')
    const personalNav = read('components/BlankNavBar.vue')
    expect(personalNav.indexOf('<template v-else>')).toBeLessThan(personalNav.indexOf('>Business login'))
  })
})
