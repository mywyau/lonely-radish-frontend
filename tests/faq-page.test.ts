...
describe('product rules and FAQ', () => {
  ...
  it('explains discovery, planning, cancellation, privacy, and safety', () => {
    ...
    expect(faq).toContain('What happens when I accept an interest?')
    expect(faq).not.toContain('manual match')
    ...
    expect(faq).toContain('Your choice and optional note stay hidden until both people answer')
    expect(faq).toContain('If you both choose another date, the connection stays open')
