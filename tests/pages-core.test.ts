...
describe('core page contracts', () => {
  ...
  it('mock activity and match pages are routed separately from the preview page', () => {
    ...
    expect(activities).toContain('Start with a date you would actually enjoy.')
    ...
    expect(activities).toContain('See potential dates')
    expect(activityMatches).toContain('Potential dates')
    expect(activityMatches).toContain('would enjoy this date idea too')
    expect(activityMatches).toContain('No activity matches found')
    expect(activityMatches).toContain('`/profiles/${person.name.toLowerCase()}`')
    ...
    expect(profile).toContain('Suggest a date with {{ profile.name }}')
    ...
    expect(plan).toContain('Pick a public venue')
