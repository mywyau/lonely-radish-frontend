...
describe('business approval workflow', () => {
  ...
  it('provides a reversible business promotion kill switch', () => {
    const migration = read('docs/migrations/20260917_add_business_intervention_kill_switch.sql')
    const endpoint = read('server/api/admin/businesses/[id]/intervention.patch.ts')
    const dashboard = read('pages/admin/businesses.vue')

    expect(migration).toContain("'paused','suspended'")
    expect(migration).toContain("'paused','restored'")
    expect(endpoint).toContain('requireAdmin(event)')
    expect(endpoint).toContain("current.status !== expectedStatus")
    expect(endpoint).toContain('set active=false')
    expect(endpoint).toContain("claim.status='issued'")
    expect(endpoint).toContain("set status='revoked'")
    expect(endpoint).toContain('admin_review_events')
    expect(read('server/api/admin/reviews.patch.ts')).toContain("status<>'paused'")
    expect(read('server/services/offerRedemption.ts')).toContain("claim.businessStatus === 'active'")
    expect(dashboard).toContain('Pause promotion')
    expect(dashboard).toContain('Restore business')
    expect(dashboard).toContain('Its offers remain inactive')
  })
})
