...
export default defineEventHandler(async (event) => {
  ...
  const { rows } = await db.query(`select storage_key,thumbnail_storage_key
    from profile_photos where id=$1 and user_id=$2`, [id, sub])
  if (!rows[0]) throw createError({ statusCode: 404, statusMessage: 'Photo not found' })
  const storageKeys = [rows[0].storage_key, rows[0].thumbnail_storage_key].filter(Boolean)
  ...
  try {
    ...
    const remaining = await client.query(`select id,public_url,storage_key,thumbnail_storage_key,alt_text
