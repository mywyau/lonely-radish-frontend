...
export default defineEventHandler(async (event) => {
  ...
  const { rows } = await db.query(`select
    ...
    (mp.dating_preferences_set=true and (mp.open_to_everyone=true or cardinality(mp.interested_genders)>0)
      and mp.no_orientation_preference=false and cardinality(mp.interested_orientations)>0) as "datingPreferences"
