...
export default defineEventHandler(async (event) => {
  ...
  const current = await db.query(`select id,public_url,storage_key,thumbnail_storage_key,alt_text
  ...
  try {
    ...
    for (const [index, id] of ids.entries()) {
      ...
      [id,sub,photo.public_url,photo.storage_key,photo.thumbnail_storage_key,photo.alt_text,index+1])
    }
    await client.query('commit')
  } catch (error) { await client.query('rollback'); throw error } finally { client.release() }
  const photoUrls = await signedPhotoUrls(ids.map(id => byId.get(id).storage_key).filter(Boolean))
  return { photos: ids.map((id, index) => {
    const photo = byId.get(id); return { id, storageKey: photo.storage_key,
