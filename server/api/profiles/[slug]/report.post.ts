...
export default defineEventHandler(async (event) => {
  ...
  const interestId = text(body.interestId, 'Interest ID', 50)
  ...
  if (interestId && !/^[0-9a-f-]{36}$/i.test(interestId)) {
    throw createError({ statusCode: 400, statusMessage: 'Invalid interest reference' })
  }
  ...
  try {
    ...
    if (interestId) {
      const interest = await client.query(`select id from daily_interests
        where id=$1 and sender_id=$2 and recipient_id=$3`, [interestId,reportedId,sub])
      if (!interest.rows[0]) {
        throw createError({ statusCode: 404, statusMessage: 'Interest reference not found' })
      }
      relatedInterestId = interest.rows[0].id
    }
    ...
    const report = await client.query(`insert into reports(
        reporter_id,reported_id,category,details,priority,related_match_id,related_interest_id
      ) values($1,$2,$3,$4,$5,$6,$7) returning id,status`,
    [sub,reportedId,category,details,priority,match.rows[0]?.id || null,relatedInterestId])
