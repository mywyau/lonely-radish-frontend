...
export default defineEventHandler(async (event) => {
  ...
  const message = text(body.message, 'Note', 500, true)!
  ...
  try {
    ...
    if (!match.rows[0]) throw createError({ statusCode: 403, statusMessage: 'Only the person who ended this connection can send a note to reopen it' })
    ...
    if (currentApology.rows[0]) throw createError({ statusCode: 409, statusMessage: 'You have already sent a note for this ended connection' })
