...
export default defineEventHandler(async (event) => {
  ...
  if (business.status !== 'active') throw createError({ statusCode: 403,
    statusMessage: 'Business approval is required before submitting venues' })
  if (!['owner', 'manager'].includes(business.role)) throw createError({ statusCode: 403,
    statusMessage: 'Owner or manager access is required to submit venues' })
  ...
  return transitionVenueSubmission(db, { businessId: business.id, venueId,
    actorId: business.userId, action: 'resubmit' })
})
